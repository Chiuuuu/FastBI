...
const app = {
    ...
    actions: {
        ...
        async addScreenData ({ commit, state }, obj) {
            ...
            screenManage.addScreen(params).then((res) => {
                if (res.code === 200) {
        ...
        async saveScreenData ({ rootGetters, state }, obj) {
            ...
            return screenManage.saveScreenNew(params).then((res) => {
                if (res.code === 200) {
                    res.msg && message.success(res.msg)
                    return true
                } else {
        ...
        async addChartData ({ dispatch, state }, obj) {
            ...
            return screenManage.addChart(params).then((res) => {
                if (res.code === 200) {
                    // res.msg && message.success(res.msg)
        ...
        async deleteChartData ({ dispatch, state, rootGetters }) {
            let chart = rootGetters.canvasMap.find((item) => item.id === rootGetters.currentSelected)
            let params = {
                id: chart.id,
                name: chart.name,
                screenId: state.screenId,
        ...
        async handleRefreshData ({ dispatch, commit }, { chart, newData }) {
            if (chart.setting.chartType === 'v-ring' || chart.setting.chartType === 'v-gauge') {
                ...
                let rows = []
                let dimensionKeys = chart.setting.apiData.dimensions.map(item => item.alias)
                dimensionKeys.forEach(item => {
                    // 根据当前维度分类得到的列表
                    let list = summary(newData, item, measureKeys[0])// 嵌套饼图度量只有一个，直接取第一个数

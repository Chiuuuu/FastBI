<template>
  <div
    ...
    <div class="navigator-line" v-show="isSelected" :style="lineRotateStyle">
      ...
      <DragLine v-if="isLine" :item="item" :selected="selected" :comHover="comHover">
        <slot></slot>
      </DragLine>
      <div v-else class="dv-com" :class="{'hovered':comHover}" style="transform: rotate(0deg);">
  ...
  export default {
    ...
    computed: {
        ...
        if (this.dragScale) {
          ...
          if (this.isLine) {
            result = {
              ...
              transform: `translate3d(${this.transformData.x}px,${this.transformData.y}px,0) rotate(${this.transformData.rotate || 0}deg)`
            }
          } else {
            result = {
              width: this.transformData.width + 'px',
      ...
      dvWrapperStyles () {
        ...
        result.transform = `translateZ(0) rotate(${this.transformData.rotate || 0}deg)`
        if (this.item.setting.config.title === '直线') {
          if (this.transformData.origin === 'left') {
    ...
    methods: {
      ...
      handleTransformStart(event, dotDirection) {
        ...
        // 缓存鼠标点击位置，会事实更新
        this.dragData.dragX = this.transformData.x * this.canvasRange + this.canvasOffsetX
        this.dragData.dragY = this.transformData.y * this.canvasRange + this.canvasOffsetY
        // 记录开始的xy
        this.dragData.startX = this.transformData.x
        this.dragData.startY = this.transformData.y
        // 记录开始的width height
        ...
        if (dotDirection === 'left') {
          // 获取cosin值
          const cos = Math.abs(Math.cos(((this.transformData.rotate || 0) / 180) * Math.PI))
          const sin = Math.abs(Math.sin(((this.transformData.rotate || 0) / 180) * Math.PI))
          const dx = this.dragData.startWidth * cos * this.canvasRange
          const dy = this.dragData.startWidth * sin * this.canvasRange
          console.log(this.dragData.dragX, dx)
          if (this.transformData.rotate > 180) {
          ...
          this.transformData.origin = 'right' // 切换形变中心
        } else { // 拖住右端点
          this.transformData.origin = 'left' // 切换形变中心
        }
        // 记录开始的圆心
        this.dragData.originX = origin.originX
        this.dragData.originY = origin.originY
        // 比较上次端点位置切换旋转角度
        // if (dotDirection === this.transformData.origin) { // 没变
        //   this.transformData.rotate = this.transformData.rotate
        // } else { // 切换了端点, 则需反转坐标轴
        //   this.transformData.rotate = (this.transformData.rotate + 180) % 360
        // }
        this.$store.dispatch('SetBaseProperty', this.transformData)
        console.log('init', this.dragData)
      ...
      handleTransformMove(event) {
        ...
        // if (this.transformData.origin === 'left') {
          ...
          const cos = Math.abs(Math.cos((this.transformData.rotate || 0) / 180 * Math.PI))
          this.transformData.width = Math.round(Math.abs(distance.x - this.dragData.originX) / cos / this.canvasRange)
        // }
        const rotate = (this.handleComputeAngle(distance, 'right') + 360) % 360
        this.transformData.rotate = +rotate.toFixed(2)
        // // 最终旋转角度增量 = 当前旋转角度 - 初始偏移角度
        // // rotate值 = (初始旋转值 + 旋转角度增量) % 360
        // const angle = this.handleComputeAngle(distance)
      ...
      handleComputeAngle(distance, direction = 'left') {
        // 减去画布到页面的距离

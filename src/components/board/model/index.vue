  ...
  methods: {
    ...
    async saveData(item) {
      let params = {
        ...
        resourceName: item.alias || item.name,
        datasourceId: '',
        databaseId: '',
        tableId: item.id,
      ...
      item.resourceName = item.alias || item.name
    ...
    async getPivoSchemaList(id) {
      ...
      if (res.code === 200) {
        ...
        dimensions = dimensions.map(item => {
          return { ...item, produceType: 0, resourceType: this.resourceType }
        })
        ...
        this.$store.dispatch('SetModelDimensions', dimensions)

<template>
  <div class="table-area scrollbar">
    <div class="area-searchbar">
      <a-form-model :model="form" layout="inline">
        <a-form-model-item label="标准地址编号" prop="addressCode">
          <a-input placeholder="请输入标准地址编号" v-model="form.addressCode" />
        </a-form-model-item>
        <a-form-model-item label="标准地址" prop="address">
          <a-input placeholder="请输入标准地址" v-model="form.address" />
        </a-form-model-item>
        <a-form-model-item label="楼盘名称编码" prop="householdNumber">
          <a-input placeholder="请输入楼盘名称编码" v-model="form.householdNumber" />
        </a-form-model-item>
        <a-form-model-item label="楼盘名称" prop="communityName">
          <a-input placeholder="请输入楼盘名称" v-model="form.communityName" />
        </a-form-model-item>
        <a-form-model-item label="标识" prop="match">
          <a-input placeholder="请输入标识" v-model="form.match" />
        </a-form-model-item>
        <a-form-model-item label="运维片区" prop="serviceSection">
          <a-input placeholder="请输入运维片区" v-model="form.serviceSection" />
        </a-form-model-item>
        <a-form-model-item label="归属分公司" prop="headOffice">
          <a-input placeholder="请输入归属分公司" v-model="form.headOffice" />
        </a-form-model-item>
        <a-form-model-item label="片区" prop="section">
          <a-input placeholder="请输入片区" v-model="form.section" />
        </a-form-model-item>
        <a-form-model-item label="网格" prop="grid">
          <a-input placeholder="请输入网格" v-model="form.grid" />
        </a-form-model-item>
        <a-form-model-item label="提供方" prop="provider">
          <a-input placeholder="请输入提供方" v-model="form.provider" />
        </a-form-model-item>
        <a-form-model-item label="清洗后地市" prop="city">
          <a-input placeholder="请输入清洗后地市" v-model="form.city" />
        </a-form-model-item>
        <a-form-model-item label="区县" prop="district">
          <a-input placeholder="请输入区县" v-model="form.district" />
        </a-form-model-item>
        <a-form-model-item label="对应街道" prop="street">
          <a-input placeholder="请输入对应街道" v-model="form.street" />
        </a-form-model-item>
        <a-form-model-item label="清洗后详细地址" prop="detailAddress">
          <a-input placeholder="请输入清洗后详细地址" v-model="form.detailAddress" />
        </a-form-model-item>
        <a-form-model-item label="是否修改过" prop="hasEdit">
          <a-select style="width: 150px" placeholder="是否修改过" v-model="form.hasEdit" allowClear>
            <a-select-option :value="1">是</a-select-option>
            <a-select-option :value="0">否</a-select-option>
          </a-select>
        </a-form-model-item>
        <a-form-model-item label="最后一次修改时间" prop="gmtModified">
          <a-range-picker valueFormat="YYYY-MM-DD HH:mm:ss" show-time :placeholder="['开始时间', '结束时间']" v-model="form.gmtModified" />
        </a-form-model-item>
        <a-form-model-item>
    ...
    <a-table
      ...
      :pagination="false"
      :scroll="{ x: 3000, y: 'calc(100vh - 450px)' }"
    >
      <!-- 除了操作之外的数据展示列 -->
      <template
      ...
      >
        <a-input v-model="editRow[col.dataIndex]" v-if="activeIndex === index" :key="i"></a-input>

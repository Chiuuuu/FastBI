  ...
  <a-spin :spinning="spinning" tip="地图加载中" class="map-area">
    ...
    <div v-if="mode === 'edit' && floor === 1" class="map-area-toolbar">
      <a-tooltip title="绘制片区">
       <a-button icon="block" :disabled="disabledToolbar" @click="handleOpenModal('draw')" />
      </a-tooltip>
      <a-tooltip title="编辑片区">
        <a-button icon="edit" :disabled="disabledToolbar" @click="handleOpenModal('edit')" />
      </a-tooltip>
      <a-tooltip title="删除片区">
        <a-button icon="delete" :disabled="disabledToolbar" @click="handleOpenModal('delete')" />
      </a-tooltip>

      <!-- 编辑模式的操作栈按钮 -->
      <div v-if="mapInstance" :class="['edit-toolbar', { show: showEdit }]" show-search :filter-option="filterOption">
        <a-button :disabled="!mapInstance.stack || mapInstance.stack.stack.length === 0" size="small" @click="mapInstance.undoStack()">撤销</a-button>
        <a-button :disabled="!mapInstance.stack || mapInstance.stack.log.length === 0" size="small" @click="mapInstance.redoStack()">恢复</a-button>
        <a-button size="small" @click="handleCloseEdit">取消</a-button>
        <a-button size="small" @click="handleSaveEdit">保存</a-button>
      </div>
    </div>

    <a-button class="map-area-back" type="primary" v-show="floor > 0 && !disabledToolbar" @click="backFloor">返回上一层</a-button>
    <a-button class="map-area-back" type="primary" v-show="showCancelDraw" @click="handleCancelDraw">取消绘制</a-button>

    <!-- 弹窗选择目标片区(绘制, 编辑, 删除) -->
    <a-modal :visible="visible" title="选择片区" @cancel="handleCancelModal" @ok="handleOk">
      <a-form-model
        ref="form"
        :model="form"
        :labelCol="{ span: 4 }"
        ...
        <a-form-model-item label="片区名称" prop="selectId">
          ...
          <a-select
            ...
            :filter-option="filterOption">
            <a-select-option v-for="item in undrawList" :key="item.id" :value="item.id">{{ item.name }}</a-select-option>
          </a-select>
          ...
          <a-select
            ...
            :filter-option="filterOption">
            <a-select-option v-for="item in drawnList" :key="item.id" :value="item.id">{{ item.name }}</a-select-option>
          </a-select>
    ...
    if (this.mode === 'edit') {
      this.contextMenu = [
        ...
        },
        {
          ...
          callback(polygon) {
            const id = polygon.getExtData().id
            if (this.showEdit && id !== self.currentArea.id) {
  ...
  methods: {
    ...
    backFloor() {
      ...
      if (index === 0) { // 行政区层
        ...
        this.mapInstance.map.setZoomAndCenter(this.floorZoomAndCenter.zoom, this.floorZoomAndCenter.center)

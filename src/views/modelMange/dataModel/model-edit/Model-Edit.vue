  ...
  <div class="Model-Edit">
    ...
    <div class="right" ref="rightBody" :class="{ 'add_new': model === 'add' }">
      ...
      <div class="description">
        <span class="d-s" :title="detailInfo.description">业务内容设定： {{detailInfo.description}}</span>
      ...
      <div class="detail scrollbar">
        <div class="detail_header">
          ...
          <div class="detail_btn">
            <a-button v-on:click="openModal('check-table')" :disabled="disableByDetailInfo">模型物化</a-button>
...
export default {
  ...
  computed: {
    ...mapState({
      modelId: state => state.modelMange.modelId, // 选中的菜单id
      addModelId: state => state.modelMange.addModelId, // 新增的模型id
      parentId: state => state.modelMange.parentId, // 选中的文件夹id
      datasource: state => state.modelMange.datasource, // 数据源
      privileges: state => state.common.privileges,
      datasourceId: state => state.modelMange.datasourceId // 数据源
    }),
    model() {
  ...
  mounted() {
    ...
    if (this.model === 'add') {
      ...
      this.$store.dispatch('modelMange/setModelId', this.$route.query.modelId)
    }
  ...
  beforeDestroy() {
    ...
    this.$store.dispatch('modelMange/setAddModelId', -1)
  },
  methods: {
    /** 组合右键菜单 */
    ...
    async handleGetAddModelDatamodel() {
      ...
      if (result.code === 200) {
        ...
        this.$store.dispatch('modelMange/setAddModelId', result.data.id)
      ...
      if (result.code === 200) {
        ...
        if (this.databaseList.length && this.databaseList.length > 0) {
          ...
          if (listResult.code === 200) {
            ...
            this.$store.dispatch('modelMange/setDatabaseId', result.data[0].id)
          } else {
    ...
    async handleSave() {
      ...
      if (result.code === 200) {
        ...
        this.$message.success({
          ...
          this.$store.commit('modelMange/SET_MODELID', result.data.id)
          this.exit()
        })
      } else {
        this.$message.error(result.msg)
      }
      this.$store.dispatch('modelMange/setParentId', '')
    },
    /**
     * 保存模型后再保存关联的数据源信息
     */

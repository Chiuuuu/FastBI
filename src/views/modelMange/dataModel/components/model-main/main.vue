...
export default {
  ...
  computed: {
    ...mapState({
      modelId: state => state.modelMange.modelId,
      modelName: state => state.modelMange.modelName,
      privileges: state => state.common.privileges,
      datasourceId: state => state.modelMange.datasourceId
    }),
    hasEditPermission() {
    ...
    */
    async handleGetData(id) {
      ...
      if (result.code === 200) {
        ...
        this.$store.commit('modelMange/SET_MODELNAME', result.data.name)
    ...
    async handleGetDataSource() {
      ...
      this.$store.dispatch('modelMange/setDatasourceId', datsource.data[0].datasourceId)

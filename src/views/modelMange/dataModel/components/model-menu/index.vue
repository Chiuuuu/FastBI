<template>
  <div class="model-menu">
    <div class="menu_title">
      ...
      <a-dropdown v-if="hasPermissionFolderAdd" :trigger="['click']" placement="bottomLeft">
      ...
      <a-button type="primary" class="model-compare-btn" @click="handleModelCompare">模型对比</a-button>
    </div>
    ...
    ></move-file-modal>
    <model-compare
      :visible="modelCompareVisible"
      :list="modelList"
      @close="handleCloseModelCompare"
    ></model-compare>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      modelCompareVisible: false, // 模型对比弹窗
      folderContenxtMenu: [
        {
          name: '新建模型',
  ...
  computed: {
    ...
    fileSelectId: {
      get () {
        return this.$store.state.modelMange.modelId
      },
      set (value) {
        this.$store.commit('modelMange/SET_MODELID', value)
      }
    },
  ...
  methods: {
    ...
    */
    ...
    async handleGetMenuList() {
      this.$store.dispatch('modelMange/getMenuList', this)
    },
    handleFolderNewModel(mouseEvent, event, { folder }) {
      this.$store.dispatch('modelMange/setParentId', folder.id)
      this.showModal()
    },
    /**
     * 打开弹窗
    ...
    async _resetName(form) {
      ...
      if (result.code === 200) {
        ...
        if (this.resetName.item.id === this.fileSelectId) {
          this.$store.commit('modelMange/SET_MODELNAME', form.name)
        }
    ...
    */
    ...
    handleFilemove(event, index, { file }) {
      ...
      this.$store.dispatch('modelMange/setDatasource', file)
    ...
    */
    ...
    handleModelCompare() {
      this.modelCompareVisible = true
    },
    handleCloseModelCompare() {
      this.modelCompareVisible = false
    }
  }
}
</script>

    ...
    <a-col class="gutter-row screen-manage" :span="24">
      <div class="main main-padding main-bg scrollbar">
        <a-row type="flex" justify="space-between" style="margin-bottom:10px">
          <a-col>
            <a-button type="primary" @click="handleEditLabel()">新建</a-button>
          </a-col>
          <a-col>
            <a-button type="primary" @click="handleOpenModal('ExamLog')">标签审核日志</a-button>
          </a-col>
        </a-row>
        ...
        <a-table
          ...
          :pagination="false"
          :scroll="{ x: 1750, y: 'calc(100vh - 320px)' }"
        >
          ...
          <template #check="list, record">
            <a @click="handleOpenModal('LabelForm', record)">详情</a>
            <a-divider type="vertical" />
            <a @click="handleOpenModal('VersionLog', record)">版本记录</a>
            <a-divider type="vertical" />
          ...
          <template #config="list, record">
            ...
            <a-divider type="vertical" />
            <a @click="handleEditLabel(record)">修改</a>
            <a-divider type="vertical" />
            <a-popconfirm
              title="是否确定删除？"
              ok-text="确定"
...
export default {
  ...
  methods: {
    ...
    formatCondition(record) {
      ...
      return `${role}【${field}】满足条件：${condition}`
    },
    // 重置表单
    handleResetForm() {
    ...
    handleEditLabel(row) {
      ...
      if (row) {
        ...
        this.$router.push({ name: 'labelEdit' })
      }
    },
    // 下线/关闭标签
